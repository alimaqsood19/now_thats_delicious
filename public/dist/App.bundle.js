...
function autoComplete(input, latInput, lngInput) {
    if (!input) return; //skips if no input on the page
    ...
    //whensomeone hits enter dont submit form
...
function ajaxHeart(e) {
    ...
    .then(function (res) {
        var isHearted = _this.heart.classList.toggle('heart__button--hearted'); //this.heart refers to the name property in the form tag (name=heart)
        (0, _bling.$)('.heart-count').textContent = res.data.hearts.length;
        if (isHearted) {
            _this.heart.classList.add('heart__button--float'); //adds the specified class for the heart float animation
            setTimeout(function () {
...
function loadPlaces(map) {
    ...
    _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng) //get request for the lat and lng passed in
    .then(function (res) {
        var places = res.data; //the data that comes back, all the documents 
        if (!places.length) {
        ...
        var markers = places.map(function (place) {
            //places array response data comes as lng, lat
            //const [placeLng, placeLat] = place.location.coordinates; //array destructuring can also do
            var placeLng = place.location.coordinates[0];
            var placeLat = place.location.coordinates[1];
            var position = {
                lat: placeLat,
                lng: placeLng
            };
        ...
        markers.forEach(function (marker) {
            //iterating through each marker in markers array, and adding a click listener
...
function makeMap(mapDiv) {
    //already have googleapi map loaded under layout.pug scripts
    if (!mapDiv) return;
    //Make the map
    var map = new google.maps.Map(mapDiv, mapOptions); //pass in the mapDiv #map ID and the mapOptions, creates the map on page
...
//sanitizes innerHTML, prevents onload javascript 
...
function typeAhead(search) {
    ...
    searchInput.on('input', function () {
        ...
        if (!this.value) {
            //if there is no value, quit all search results, and stop search from running
        ...
        axios.get('/api/search?q=' + this.value).then(function (res) {
            if (res.data.length) {
                var html = _dompurify2.default.sanitize(searchReultsHTML(res.data)); //res.data is the resulting stores from the get request
    ...
    searchInput.on('keyup', function (event) {
        ...
        if (![38, 40, 13].includes(event.keyCode)) {
            //keycodes 38, 40, 13 is up down enter 
            return;
        }
        ...
        if (event.keyCode === 40 && current) {
            //if they press down keyCode = 40, and they have selected one 
            ...
            //if its just down then its the next item
            next = items[0];
            ...
            //when they hit enter, and there is a current element with a href value on it, 
